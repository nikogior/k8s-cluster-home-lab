...
export POD_NAME=$(kubectl get pods -l app=nginx \
...
export NODE_PORT=$(kubectl get svc nginx \
  --output=jsonpath='{range .spec.ports[0]}{.nodePort}')

# Make an HTTP request using the IP address and the nginx node port:
export NODE_NAME=$(kubectl get pod $POD_NAME --output=jsonpath='{.spec.nodeName}')
